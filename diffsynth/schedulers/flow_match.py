...
class FlowMatchScheduler():
    ...
    def step_(self, model_output, timestep, sample, to_final=False, **kwargs):
        if isinstance(timestep, torch.Tensor):
            timestep = timestep.cpu()
        timestep_id = torch.argmin((self.timesteps - timestep).abs())
        sigma = self.sigmas[timestep_id]
        x0_preds = sample - sigma * model_output
        return x0_preds    
    ...
    def add_noise(self, original_samples, noise, timestep):
        ...
        sigma = self.sigmas[timestep_id].reshape(-1, 1, 1, 1, 1).to(noise.device)
    ...
    def training_weight(self, timestep):
        timestep_id = torch.argmin((self.timesteps.unsqueeze(1) - timestep.to(self.timesteps.device).unsqueeze(0)).abs(), dim=0)
