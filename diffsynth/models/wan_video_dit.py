"""
We modify this file from the original version.
"""
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False):
    if (not compatibility_mode) and FLASH_ATTN_2_AVAILABLE:
...
def rope_apply(x, freqs, num_heads, phase=None):
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, cam_emb=None):
        if cam_emb is not None:
            # camera embeeding conditioned RoPE phase shift
            rope_phase_qk = self.rope_phase_qk(cam_emb)
            rope_phase_vo = self.rope_phase_vo(cam_emb)
            q = self.norm_q(self.q(x))
            k = self.norm_k(self.k(x))
            v = self.v(x)
            q = rope_apply(q, freqs, self.num_heads, rope_phase_qk)
            k = rope_apply(k, freqs, self.num_heads, rope_phase_qk)
            v = rope_apply(v, None, self.num_heads, -rope_phase_vo)
            x = self.attn(q, k, v)
            x = self.o(x)
            x = rope_apply(x, None, self.num_heads, rope_phase_vo)
            return x
        else:
            q = self.norm_q(self.q(x))
            k = self.norm_k(self.k(x))
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, cam_emb, t_mod, freqs):
        ...
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs, cam_emb))
...
class WanModel(torch.nn.Module):
    ...
    def patchify(self, x: torch.Tensor, control_camera_latents_input: Optional[torch.Tensor] = None):
        ...
        if self.control_adapter is not None and control_camera_latents_input is not None:
            x_camera = self.control_adapter(control_camera_latents_input)
            x = x + x_camera
            grid_size = x.shape[2:]
